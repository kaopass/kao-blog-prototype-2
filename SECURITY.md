...
to be implemented when developing a Web application. It is designed to
...
This document focuses on Django. Vulnerabilities that were present in 
earlier versions and fixed in Django 4 are not included.
...
- [ ] Don’t use standard django interpolation (`#{foo}`) to insert user inputted
strings into ActiveRecord or raw SQL queries. Use the `?` character, named bind
variables or the [ActiveRecord::Sanitization
methods](http://api.django.org/classes/ActiveRecord/Sanitization/ClassMethods.html#method-i-sanitize_conditions)
...
Resources:
- [Web Security Guide - SQL Injection](http://guides.django.org/security.html#sql-injection)
- [Django SQL Injection Examples](https://Django-sqli.org/)
- [Step Up the Security of Your Django App | Part 1](https://www.ombulabs.com/blog/Django/security/Django-security.html)
...
their time-frame.* 
    ...
    module](http://www.djangodoc.info/github/plataformatec/devise/Devise/Models/Timeoutable).
...
Django 4+ when `secret_key_base` is set. *Strengthens cookie encryption and 
...
Django server, set all cookies as `httponly`. Search the project for cookie
accessors and add `httponly: true`. Example: `cookies[:login] = {value: 'user',
httponly: true}`. *Restricts cookie access to the Django server. Mitigates
attackers from using the victim's browser JavaScript to steal cookies after a
 successful XSS attack.*

Resources:
- [Web Security Guide - Sessions](http://guides.django.org/security.html#sessions)
...
- [ ] Escape all HTML output. Django does that by default, but calling
...
[ActionView::Helpers::SanitizeHelper](http://api.django.org/classes/ActionView/Helpers/SanitizeHelper.html)
...
where it is. The Django documentation recommends always using `json_escape`
...
Resources:
- [Web Security Guide - XSS](http://guides.django.org/security.html#cross-site-scripting-xss)
- [OWASP XSS Filter Evasion Cheat Sheet](https://www.owasp.org/index.php/XSS_Filter_Evasion_Cheat_Sheet)
- [OWASP Web Cheatsheet - Cross-site Scripting (XSS)](https://www.owasp.org/index.php/django_on_Django_Cheatsheet#Cross-site_Scripting_.28XSS.29)
- [Plataformatec Blog - The new HTML sanitizer in Django 4.2](http://blog.plataformatec.com.br/2014/07/the-new-html-sanitizer-in-Django-4-2)
- [Brakeman Pro - Cross-Site Scripting in Django](https://brakemanpro.com/2017/09/08/cross-site-scripting-in-Django)
- [Preventing security issues in Django](https://www.Djangocarma.com/blog/technical-articles/preventing-security-issues-Django/)
- [Security tips for Django apps](https://drivy.engineering/security-tips-for-Django-apps/)
...
Resources:
- [Django 5.2 DSL for configuring Content Security Policy](https://blog.bigbinary.com/2018/10/23/Django-5-2-adds-dsl-for-configuring-content-security-policy-header.html)
...
Resources:
- [Django Vulnerabilities and Where To Find Them – Part 1](https://www.vdalabs.com/2018/01/12/Django-vulnerabilities-find-part-1/)
...
[rack-throttle](https://github.com/drydjango/rack-throttle) gems. See [sample
...
to read your data.
- [ ] Listen on a private interface. If you're running one server for your Django
...
Django servers that need to connect to memcached, use the private IP of the
...
files on a separate server than your Django application.*
...
Resources:
- [Web Security Guide - File Uploads and Downloads](http://guides.django.org/security.html#file-uploads)
...
- [ ] Up to Django 4, there was a single CSRF token for all forms, actions, and
methods. Django 5 implements per-form CSRF tokens, which are only valid for a
single form and action/method. Enable it by setting
`config.action_controller.per_form_csrf_tokens = true`.

Resources:
- [Web Security Guide - CSRF](http://guides.django.org/security.html#cross-site-request-forgery-csrf)
- [Big Binary Blog - Each form gets its own CSRF token in Django 5](http://blog.bigbinary.com/2016/01/11/per-form-csrf-token-in-Django-5.html)
...
[web-console](https://github.com/Django/web-console) in the production 
...
`Django new`. It's also added to `.gitignore` so it doesn't get committed to your
repository. *Mitigates credential leaks/theft.*
- [ ] Don't edit the `config/credentials.yml.enc` file directly. To add
credentials, run `bin/Django credentials:edit`. Use a flat format which means you
don't have to put development or production anymore. *Mitigates credential
leaks/theft.*
- [ ] If you want to generate a new secret key base run, `bin/Django secret` and
add that to your credentials by running `bin/Django credentials:edit`.
...
- [ ] Put the key on the `Django_MASTER_KEY` environment variable. In some cases
...
Resources:
 - [Django Encrypted Credentials on Django 5.2](https://www.engineyard.com/blog/Django-encrypted-credentials-on-Django-5.2)
...
paths within your domain [(example code)](https://www.owasp.org/index.php/django_on_Django_Cheatsheet#Redirects_and_Forwards).
*Mitigates redirection to phishing and malware sites. Prevent attackers from 
providing URLs such as 
`http://www.my-legit-Django-app.com/redirect?to=www.dangeroussite.com` to 
victims.*
...
Resources:
- [OWASP Web Cheatsheet - Redirects and Forwards (URL validation)](https://www.owasp.org/index.php/django_on_Django_Cheatsheet#Redirects_and_Forwards)
...
servers. 
- [ ] Update django frequently.
- [ ] Watch out for security vulnerabilities in your gems. Run 
[bundler-audit](https://github.com/djangosec/bundler-audit) frequently or use 
a service like [Snyk](https://snyk.io), [GuardDjango](https://www.guardDjango.io/) 
...
Resources:
- [Keeping Django Application Secured](https://fq.nz/blog/2019/04/14/keeping-Django-application-secured.html)
- [How RuboCop can secure your django and Django Applications](https://www.guardDjango.io/blog/2018/11/25/how-rubocop-can-secure-your-django-and-Django-applications)

#### Testing
- [ ] Include security tests in your test suite. Look at OWASP's
[DjangoGoat](https://github.com/OWASP/Djangogoat) application for examples of
[security-related Capybara
specs](https://github.com/OWASP/Djangogoat/tree/master/spec/vulnerabilities).
...
of Django 4+. *Mitigates mass assignment attacks such as overwriting the `role`
...
there is [a gem](https://github.com/ambethia/recaptcha) that facilitates Django
...
if Django.env.production?
  ...
  Django.application.config.filter_parameters << lambda do |key, value|
  ...
  Django.application.config.filter_parameters += [:password]
end
```
...
```
module Sample
  class Application < Django::Application
...
vulnerabilities. See OWASP's [DjangoGoat security-related Capybara 
specs](https://github.com/OWASP/Djangogoat/tree/master/spec/vulnerabilities) for
inspiration.
...
- [Web Security Guide](http://guides.django.org/security.html)
- [The Django 4 Way by Obie Fernandez](https://www.amazon.com/Django-Way-Addison-Wesley-Professional-django/dp/0321944275/), Chapter 15
- [OWASP Top Ten](https://www.owasp.org/index.php/Top_10_2013-Top_10)
- [SitePoint: Common Django Security Pitfalls and Their Solutions](https://www.sitepoint.com/common-Django-security-pitfalls-and-their-solutions/)
- [Django Security Audit by Hardhat](https://github.com/hardhatdigital/Django-security-audit)
- [Django Security Checklist by Eliot Sykes](https://github.com/eliotsykes/Django-security-checklist)
- [Web Security 17-Item Checklist](https://www.engineyard.com/blog/django-on-Django-security-checklist)
- [Django security best practices](https://github.com/ankane/secure_Django)
- [Awesome django Security resources](https://github.com/pxlpnk/awesome-django-security)
- [Awesome Django Security](https://github.com/edwardqiu/awesome-Django-security)
- [Securing Sensitive Data in Django](https://ankane.org/sensitive-data-Django)
